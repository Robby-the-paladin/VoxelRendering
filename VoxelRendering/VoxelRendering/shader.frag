...
in vec4 gl_FragCoord; // êîîðäèíàòû ôðàãìåíòà
...
struct Camera {
    vec2 resolution; // ðàçðåøåíèå ýêðàíà
    vec3 pos; // ïîëîæíèå êàìåðû â êîîðäèíòàõ ìèðà
    vec3 dir; // âåêòîð íàïðàâëåíèÿ êàìåðû
    float render_distance; // äàëüíîñòü ïðîðèñîâêè
    float viewing_angle; // óãîë îáçîðà
    // float tilt_angle; // óãîë íàêëîíà êàìåðû (íå èñïîëüçóåòñÿ default 90)
};
...
Raylaunching_response raylaunching(vec3 beg, vec3 end) {
    ...
    while(top_num != -1) {
        ...
        if (tree[node_num].terminal_empty_texture_using[0] != 0) {
            if (tree[node_num].terminal_empty_texture_using[1] != 0) { // åñëè ïóñòàÿ âåðøèíà âîçðàùàòü -1 â node_num
                FragColor = vec4(0, 1, 0, 0);
                continue;
            }
            else { // èíà÷å íîìåð node_num è beg â point
...
void main() {
    ...
    // íà÷àëî òðàññèðóåìîãî îòðåçêà
    vec3 beg = cam.pos; // ïîçèöèÿ êàìåðû

    // êîíåö òðàññèðóåìîãî îòðåçêà
    vec2 coords = gl_FragCoord.xy - vec2(0.5, 0.5) * cam.resolution; // ïîëîæåíèå ïèêñåëÿ íà ýêðàíå (öåíòð ýêðàíà (0, 0))

    vec3 old_x = normalize(cross(cam.dir, vec3(0, 0, 1))); // åäèíè÷íûé âåêòîð Ox äëÿ ýêðàíà â ïðîñòðàíñòâå (x â ñòàðîì áàçèñå)
    vec3 old_z = cam_dir; // åäèíè÷íûé âåêòîð Oz äëÿ ýêðàíà â ïðîñòðàíñòâå (z â ñòàðîì áàçèñå)
    vec3 old_y = normalize(cross(old_x, old_z)); // åäèíè÷íûé âåêòîð Oy äëÿ ýêðàíà â ïðîñòðàíñòâå (y â ñòàðîì áàçèñå)
    vec3 old_O = cam.pos; // öåíòð êîîðèíàò ñòàðîãî ÑÊ â íîâîì áàçèñå

    float cam_dist = tan((M_PI  - cam.viewing_angle) / 2.0) * cam.resolution.x / 2; // ðàññòîÿíèå îò íàáëþäàòåëÿ äî ýêðàíà
    vec3 old_point = vec3(coords, cam_dist); // ïîëîæåíèå òî÷êè â ïðîñòðàñòâå, ÷åðåç êîòîðóþ ïðîéä¸ò ëó÷, â ñòàðîì áàçèñå

    // òî÷êà â ïðîñòðàñòâå, ÷åðåç êîòîðóþ ïðîéä¸ò ëó÷
    vec3 point =  old_x * old_point.xxx + old_y * old_point.yyy + old_z * old_point.zzz + old_O;

    

    vec3 end = normalize(point - beg) * cam.render_distance + beg; // êîíåö òðàññèðóåìîãî îòðåçêà ñ ó÷¸òîì äàëüíîñòè ïðîðèñîâêè

    // íîìåð âåðøèíû â êîòîðóþ ïîïàë ëó÷ (èëè -1) è òî÷êà â êîòîðóþ ïîïàë ëó÷ (èëè êîíåö îòðåçêà)
